...
def _print_progress_if_better(summary: dict[str, object],
    ...
    if sp > best["spearman_value"]:
        ...
        print(f"[NEW BEST Spearman] {summary['score_name']:<35} "
              f"sp={sp:.4f}  winner={wm}/15  top5_overlap={overlap:.4f}", flush=True)
    if wm > best["winner_value"] or (wm == best["winner_value"] and sp > best["winner_tiebreak"]):
        ...
        print(f"[NEW BEST Winner ] {summary['score_name']:<35} "
              f"winner={wm}/15  sp={sp:.4f}  top5_overlap={overlap:.4f}", flush=True)
...
def main() -> None:
    ...
    md_lines = [
        ...
        f"- `avg_vbench_z_winner_match = {sp_row['avg_vbench_z_winner_match']}/15`",
        ...
        f"- `avg_vbench_z_winner_match = {wm_row['avg_vbench_z_winner_match']}/15`",
        ...
        "",
        "| name | spearman | winner /15 | top5_overlap | subj_sp |",
        "| --- | ---: | ---: | ---: | ---: |",
    ]
    ...
    md_lines.append("")
    md_lines.append("| name | winner /15 | spearman | top5_overlap | subj_sp |")
    md_lines.append("| --- | ---: | ---: | ---: | ---: |")
    top15w = sorted(
        rows,
