...
on (a) vbench_quality prompt-mean Spearman and (b) vbench_quality winner_match.
"""
...
def _evaluate(seed_records: list[dict[str, object]],
    ...
    for row in seed_records:
        ...
        for metric in SUBSCORES + ["vbench_quality"]:
...
def _print_progress_if_better(summary: dict[str, object],
    ...
    sp = summary["vbench_quality_prompt_mean_spearman"]
    wm = summary["vbench_quality_winner_match"]
    overlap = summary["vbench_quality_top5_overlap_rate"]
...
def main() -> None:
    ...
    md_lines = [
        ...
        "",
        "## Best by vbench_quality prompt-mean Spearman",
        f"- name: `{sp_row['score_name']}`",
        f"- `vbench_quality_prompt_mean_spearman = {sp_row['vbench_quality_prompt_mean_spearman']:.4f}`",
        f"- `vbench_quality_winner_match = {sp_row['vbench_quality_winner_match']}/{n_prompts}`",
        f"- `vbench_quality_top5_overlap_rate = {sp_row['vbench_quality_top5_overlap_rate']:.4f}`",
        f"- `subject_consistency_prompt_mean_spearman = {sp_row['subject_consistency_prompt_mean_spearman']:.4f}`",
        "",
        "## Best by vbench_quality winner_match",
        f"- name: `{wm_row['score_name']}`",
        f"- `vbench_quality_winner_match = {wm_row['vbench_quality_winner_match']}/{n_prompts}`",
        f"- `vbench_quality_prompt_mean_spearman = {wm_row['vbench_quality_prompt_mean_spearman']:.4f}`",
        f"- `vbench_quality_top5_overlap_rate = {wm_row['vbench_quality_top5_overlap_rate']:.4f}`",
        "",
        "## Top 15 by vbench_quality prompt-mean Spearman",
        "",
        f"| name | spearman | winner /{n_prompts} | top5_overlap | subj_sp |",
        "| --- | ---: | ---: | ---: | ---: |",
    ]
    top15 = sorted(rows, key=lambda r: r["vbench_quality_prompt_mean_spearman"], reverse=True)[:15]
    for r in top15:
        md_lines.append(
            f"| `{r['score_name']}` | {r['vbench_quality_prompt_mean_spearman']:.4f} | "
            f"{r['vbench_quality_winner_match']} | {r['vbench_quality_top5_overlap_rate']:.4f} | "
            f"{r['subject_consistency_prompt_mean_spearman']:.4f} |"
        )
    md_lines.append("")
    md_lines.append("## Top 15 by vbench_quality winner_match")
    md_lines.append("")
    md_lines.append(f"| name | winner /{n_prompts} | spearman | top5_overlap | subj_sp |")
    md_lines.append("| --- | ---: | ---: | ---: | ---: |")
    top15w = sorted(
        rows,
        key=lambda r: (r["vbench_quality_winner_match"], r["vbench_quality_prompt_mean_spearman"]),
        reverse=True,
    )[:15]
    for r in top15w:
        md_lines.append(
            f"| `{r['score_name']}` | {r['vbench_quality_winner_match']} | "
            f"{r['vbench_quality_prompt_mean_spearman']:.4f} | "
            f"{r['vbench_quality_top5_overlap_rate']:.4f} | "
            f"{r['subject_consistency_prompt_mean_spearman']:.4f} |"
        )
